...
实现了数论变换NTT，用于处理环Z_q[x]/(phi)中的多项式运算
"""
from falconIBS.common import split, merge, q                     # Import split and merge
from falconIBS.ntt_constants import roots_dict_Zq, inv_mod_q     # Import constants useful for the FFT
...
sqr1 = roots_dict_Zq[2][0] # 模𝑞下−1的平方根，当前取值为1479，这意味着1479^2 ≡ −1 mod 12289
...
def split_ntt(f_ntt):
    ...
    将多项式 f_ntt 分割成两个部分 f0 和 f1, 使用 2 的逆元和根进行变换
    """
    n = len(f_ntt)
    w = roots_dict_Zq[n]
    f0_ntt = [0] * (n // 2)
