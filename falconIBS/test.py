...
def vecmatmul(t, B):
    ...
    Format: coefficient
    向量与矩阵乘法，给定向量t和矩阵B，计算t * B
    """
    nrows = len(B)
    ncols = len(B[0])
    deg = len(B[0][0])
    assert(len(t) == nrows) # 维度检查
...
def test_fft(n, iterations=10):
    """
    Test the FFT.
    测试快速傅里叶变换的相关运算，生成两个随机向量f和g，计算乘积h，然后除以f得到k，如果结果与g不匹配则打印错误信息
    """
...
def check_ntru(f, g, F, G):
    """
        ...
        检查两个多项式f * G - g * F = q mod (x ** n + 1)，用于验证NTRU加密方案的正确性
    """
    a = karamul(f, G)
    b = karamul(g, F)
...
def test_ntrugen(n, iterations=10):
    """
    Test ntru_gen.
    测试ntrugen是否生成符合NTRU加密要求的公钥和私钥对
    """
    for i in range(iterations):
        f, g, F, G = ntru_gen(n)
...
def test_ffnp(n, iterations):
    ...
    检查两种版本的 ffnp（系数版本和FFT嵌入版本）是否一致，并验证输出的格向量是否接近目标
    """
    f = sign_KAT[n][0]["f"]
    g = sign_KAT[n][0]["g"]
    F = sign_KAT[n][0]["F"]
...
def test_samplerz(nb_mu=100, nb_sig=100, nb_samp=1000):
    ...
    see ia.cr/2019/1411.
    测试高斯采样器的性能和准确性，通过生成大量的样本并进行卡方检验，确保采样器输出的样本符合预期
    """
...
def test_sign_KAT():
    ...
    使用测试向量（KATs）验证Falcon签名方案的正确性，确保私钥和消息生成的签名与预期一致
    """
    message = b"data1"
